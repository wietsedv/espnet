...
if [ ! -e "${PV_SPRAAK}" ]; then
    log "Fill the value of 'PV_SPRAAK' of db.sh"
    exit 1
fi
pv_root=${PV_SPRAAK}
...
if [ ${stage} -le 1 ] && [ ${stop_stage} -ge 1 ]; then
    ...
    cut -d "|" -f 1 < ${pv_root}/metadata.csv | while read -r id; do
        filename="${pv_root}/wavs/${id}.wav"
    ...
    paste -d " " \
        <(cut -d "|" -f 1 < ${pv_root}/metadata.csv) \
        <(cut -d "|" -f 3 < ${pv_root}/metadata.csv) \
        > ${text}
